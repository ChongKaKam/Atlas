...
Atlas 是工作流 Skill，配有只读知识检查工具和独立的配置/升级工具。LLM 决定如何组织知识，工具检查能机械判定的结构。所有正式知识保存在用户指定的 Markdown 目录。
...
初始化时请用户指定全局知识库绝对路径，再按 [用户配置与升级](user-management.md) 保存默认位置。后续可配置项目覆盖，也可仅为本次请求显式指定路径。用户只要求一次性操作时不强制保存全局配置。

解析知识库后，读取该路径下的 README 和 CONVENTIONS。新库初始化创建两个根文件、布局注册表和空 shared，再填写范围与入口。首次添加知识提示用户选择项目/领域，用 scopes 命令注册需要的分区；不要把整套空领域树复制进去。旧库不自动搬迁，详见 [分区与审核](scopes.md)。
...
- “使用 Atlas，初始化全局知识库到 `/path/to/kb`，这是一个新目录。”
- “使用 Atlas，把当前项目的知识库设置为 `/path/to/project/docs/knowledge`。”
- “使用 Atlas，查询默认知识库中的 LLVM dialect 笔记。”
- “更新 Atlas Skill 到已配置远端的最新版本，并检查当前知识库兼容性。”
- “Atlas update，拉取新版并对比功能变化，保留我的本地场景要求。”
- “将这份材料沉淀到 llvm 领域，使用本库 `.atlas/capture-profiles/code-reading.md` 的额外要求。”
- “从 compiler-x 提议可复用的共享知识，先给我 shared 的完整草案，等我审核。”
